<div class="page">
                ...
                <tbody>
                    @forelse ($schedules as $schedule)
                        <tr wire:key="schedule-{{ $schedule->id }}">
                            ...
                            <td class="tabular-nums">{{ substr($schedule->departure_time, 0, 5) }}–{{ substr($schedule->arrival_time, 0, 5) }}</td>
                            <td class="tabular-nums">{{ $schedule->valid_from->format('Y-m-d') }} → {{ $schedule->valid_to?->format('Y-m-d') ?? '∞' }}</td>
                                ...
                                @can('manage-schedules')
                                    ...
                                    @if ($schedule->status !== 'cancelled')
                                        <button wire:click="cancel({{ $schedule->id }})"
                                                wire:confirm="Cancel this schedule?"
                                                class="link-warn">Cancel</button>
                                    @endif
                    ...
                    @empty
                        <tr><td colspan="9" class="empty">No schedules yet.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if ($schedules->hasPages())<div class="table-foot">{{ $schedules->links() }}</div>@endif
    </div>
            ...
            <div class="modal-panel modal-lg">
                ...
                <div class="modal-body">
                    ...
                    <div>
                        ...
                        @error('bus_route_id') <p class="error-text">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="form-grid-3">
                        ...
                        <div>
                            ...
                            @error('arrival_time') <p class="error-text">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div class="form-grid-3">
                        <div>
                            ...
                            @error('valid_from') <p class="error-text">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="label">Valid to <span class="font-normal text-zinc-400 dark:text-zinc-500">(optional)</span></label>
                            <input type="date" wire:model="valid_to" class="input">
                            @error('valid_to') <p class="error-text">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="label">Status</label>
    ...
    @if ($tripsSchedule)
        <div class="modal-backdrop">
            <div class="modal-panel modal-lg">
                <div class="modal-head">
                    <h2 class="modal-title">Trips — {{ $tripsSchedule->route?->code }} · {{ substr($tripsSchedule->departure_time, 0, 5) }}–{{ substr($tripsSchedule->arrival_time, 0, 5) }}</h2>
                ...
                <div class="modal-body">
                    ...
                    @forelse ($tripsSchedule->trips as $trip)
                        <div wire:key="trip-{{ $trip->id }}"
                            ...
                            <span class="flex items-center gap-3 text-sm">
                                <span class="tabular-nums text-zinc-700 dark:text-zinc-200">{{ $trip->trip_date->format('Y-m-d') }}</span>
                                @php
                            ...
                            <select wire:change="updateTripStatus({{ $trip->id }}, $event.target.value)" class="input w-40">
                    ...
                    @empty
                        <div class="callout-muted">No trips generated yet — use the button below to create them from this schedule's validity window.</div>
                    @endforelse
                </div>
                <div class="modal-foot justify-between">
                    <button wire:click="generateTrips({{ $tripsSchedule->id }})" class="btn-ghost">Generate trips</button>
