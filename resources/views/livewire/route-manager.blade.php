<div class="page">
    ...
    <div class="card">
                ...
                <tbody>
                    @forelse ($routes as $route)
                        <tr wire:key="route-{{ $route->id }}">
                            ...
                            <td>
                                <span
                                    class="badge {{ $route->status === 'active' ? 'badge-green' : 'badge-zinc' }}">{{ $route->status }}</span>
                            </td>
                            <td class="td-actions">
                                ...
                                @can('manage-routes')
                                    ...
                                    <button wire:click="delete({{ $route->id }})"
                                        wire:confirm="Delete this route and its stops?" class="link-danger">Delete</button>
                                @endcan
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="empty">No routes yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @if ($routes->hasPages())
            <div class="table-foot">{{ $routes->links() }}</div>
        @endif
    </div>
    ...
    @if ($managingRoute)
        <div class="modal-backdrop">
            <div class="modal-panel modal-lg">
                ...
                <div class="modal-body">
                    <ol
                        class="divide-y divide-zinc-100 rounded-lg border border-zinc-200 dark:divide-zinc-800 dark:border-zinc-700">
                        @forelse ($stops as $stop)
                            <li wire:key="stop-{{ $stop->id }}"
                                ...
                                <span class="flex min-w-0 items-center gap-3 text-zinc-700 dark:text-zinc-200">
                                    <span
                                        class="flex size-6 shrink-0 items-center justify-center rounded-full bg-indigo-50 font-mono text-xs text-indigo-600 dark:bg-indigo-900/40 dark:text-indigo-300">{{ $stop->sequence }}</span>
                                ...
                                @can('manage-routes')
                                    <span class="flex shrink-0 items-center gap-2">
                                        ...
                                        <button wire:click="moveStopDown({{ $stop->id }})"
                                            @disabled($loop->last)
                                            class="link-muted disabled:cursor-default disabled:opacity-30">↓</button>
                                        <button wire:click="removeStop({{ $stop->id }})"
                                            class="link-danger text-xs">Remove</button>
                                    </span>
                                @endcan
                            </li>
                        @empty
                            <li class="px-4 py-8 text-center text-sm text-zinc-400 dark:text-zinc-500">No stops yet.
                            </li>
                        @endforelse
                    </ol>
                ...
                <div class="modal-body">
                    @if ($mapStops->isEmpty())
                        <div class="callout-muted">
                            No stops have coordinates yet. Add latitude/longitude to this route's stops to see them on
                            the map.
                        </div>
                    @else
                        <div wire:ignore wire:key="map-{{ $mapRoute->id }}" x-data="routeMap(@js($mapStops->map(fn($s) => ['name' => $s->name, 'seq' => $s->sequence, 'lat' => (float) $s->latitude, 'lng' => (float) $s->longitude])->values()), @js($mapsKey))"
                            x-init="load()">
                            <div x-ref="map"
                                class="h-80 w-full rounded-lg border border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800">
                            </div>
                        </div>
                    @endif
                </div>
        ...
        <script>
            ...
            Alpine.data('routeMap', (stops, apiKey) => ({
                ...
                loadLeaflet() {
                    ...
                    window.addEventListener('leaflet-ready', () => this.initLeaflet(), {
                        once: true
                    });
                },
                initLeaflet() {
                ...
                loadGoogle() {
                    ...
                    window.addEventListener('gmaps-ready', () => this.initGoogle(), {
                        once: true
                    });
                },
                initGoogle() {
