                ...
                <tbody>
                    @forelse ($logs as $log)
                        <tr wire:key="fuel-{{ $log->id }}">
                            ...
                            <td class="td-num">{{ $log->odometer !== null ? number_format($log->odometer) . ' km' : '—' }}</td>
                            <td class="td-actions">
                    ...
                    @empty
                        <tr><td colspan="6" class="empty">No fuel logs found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-grid-2">
                        <div>
                            ...
                            @error('liters') <p class="error-text">{{ $message }}</p> @enderror
                        </div>
                        <div>
                    ...
                    <div class="form-grid-2">
                        <div>
                            ...
                            @error('odometer') <p class="error-text">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="label">Date</label>
                            <input type="date" wire:model="logged_at" class="input">
                            @error('logged_at') <p class="error-text">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div>
                        <label class="label">Trip <span class="font-normal text-zinc-400 dark:text-zinc-500">(optional)</span></label>
                        ...
                        @error('trip_id') <p class="error-text">{{ $message }}</p> @enderror
                    </div>
                </div>
